...
# the concept of methods themselves, and how they are useful for a
# programmer. In this set of challenges, we will explore the guiding principles
# behind methods, learn more features and how to use them efficiently in our
# programs.

# In simpler terms, a method is a group of several expressions (block, so to
# say) that can be referred with a name, can be passed some arguments (input)
# and are associated with one or more objects.
...
# last part which talks about association with one or more objects. It might be
# a bit non trivial to comprehend since Classes haven't been introduced, but
# what it means is that these methods, even though they appear like global
# functions, are instead private methods of a root Object class on which they
# are implicitly defined and invoked automatically.
...
# (e.g., Python) are not a first-class citizens, because they cannot be passed
# to other methods as arguments, returned by other methods, or assigned to
# variables.
