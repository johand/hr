...
# * String.include?(string) - Returns true if str contains the given string or
#   character. Very simple!

# > "hello".include? "lo"   #=> true
# > "hello".include? "ol"   #=> false

# * String.gsub(pattern, <hash|replacement>) - Returns a new string with all the
#   occurrences of the pattern substituted for the second argument: . The
...
# * mask_article which appends strike tags around certain words in a text. The
#   method takes 2 arguments: A string and an array of words. It then replaces
#   all the instances of words in the text with the modified version.

# * A helper method strike, given one string, appends strike off HTML tags
...
def mask_article(str, arr)
  arr.each { |w| str.gsub!(w, strike(w)) }
  str
end
