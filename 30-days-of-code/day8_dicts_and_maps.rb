...
# of queries. Each line (query) contains a name to look up, and you must
...
gets.to_i.times do
  ...
  phone_book.store(input[0], input[1])
end

phone_book.select do
  k = gets.chomp
  puts phone_book.key?(k) ? "#{k}=#{phone_book[k]}" : 'Not found'
  redo
end
