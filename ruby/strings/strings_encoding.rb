...
# Until Ruby 1.8, Strings were nothing but a collection of bytes. Data was
# indexed by byte count, size was in terms of number of bytes, and so on. Since
# Ruby 1.9, Strings have additional encoding information attached to the bytes
# which provides information on how to interpret them. For example, this code:
...
# * The string has attached encoding information indicating it's an UTF-8
#   string.
# * A String's size corresponds to the umber of characters we see.
# * A String's bytesize corresponds to the actual space taken by the characters
#   in memory (the ♥ symbol requires 3 bytes instead of 1).

# Although UTF-8 is the most popular (and recommended) encoding style for
# content, Ruby supports 100 other encodings (try puts Encoding.list for the
# full list). With this in mind, we should learn how to convert between
# different encodings.

# Task

# In this challenge, we practice setting the encoding information for some
# string of text using Ruby's Encoding methods. Write a function named transcode
# which takes a ISO-8859-1 encoded string as a parameter, converts it to an
