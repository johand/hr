...
# Ruby 2.0 introduced a lazy enumeration feature. Lazy evaluation increases
# performance by avoiding needless calculations, and it has the ability to
# create potentially infinite data structures.
...
# If we remove lazy from the above code, then our code would try to compute all
# x ranging from 1 to Float::INFINITY.
# To avoid timeouts and memory allocation exceptions, we use lazy. Now, our code
# will only compute up to first(array_size).
